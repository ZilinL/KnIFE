...
class DeepConveNet(nn.Module):
    ...
    def output_feature_dim(self):
        data_tmp = torch.rand(1, 1, self.channels, self.points)
        EEGNetBackbone_tmp = DeepConveNet(self.channels, self.points, kernel_length=self.kernel_length, kernel_length2=self.kernel_length2, F1=self.F1, F2=self.F2, D=self.D, dropout_rate=0.5)
...
class DeepConveNet_4(nn.Module):
    def __init__(self, channels, points, kernel_length=128, kernel_length2=64, F1=8, F2=16, D=2, dropout_rate=0.5):
        super(DeepConveNet_4, self).__init__()
        self.F1 = F1
        self.F2 = F2
        self.D = D
        self.channels = channels
    ...
    def output_feature_dim(self):
        data_tmp = torch.rand(1, 1, self.channels, self.points)
        EEGNetBackbone_tmp = DeepConveNet_4(self.channels, self.points, kernel_length=self.kernel_length, kernel_length2=self.kernel_length2, F1=self.F1, F2=self.F2, D=self.D, dropout_rate=0.5)
        EEGNetBackbone_tmp.eval()
        _feature_dim = EEGNetBackbone_tmp(data_tmp).view(-1,1).shape[0]
        return _feature_dim

class DeepConveNet_6(nn.Module):
    def __init__(self, channels, points, kernel_length=128, kernel_length2=64, F1=8, F2=16, D=2, dropout_rate=0.5):
        super(DeepConveNet_6, self).__init__()
        self.F1 = F1
        self.F2 = F2
        self.D = D
        self.channels = channels
        ...
        # Block1
        self.conv0 = Conv2dWithConstraint(1, 25, (1,5), max_norm=2, padding=0, bias=False)
        self.conv1 = Conv2dWithConstraint(25, 25, (self.channels, 1), max_norm=2, padding=0, bias=False)
        self.batchnorm1 = nn.BatchNorm2d(25)
        self.activate1 = nn.ELU()
        self.maxpool1 = nn.MaxPool2d((1,3), stride=(1,3))
        ...
        #Block2
        self.conv2 = Conv2dWithConstraint(25, 50, (1,5), max_norm=2, padding=0, bias=False)
        self.batchnorm2 = nn.BatchNorm2d(50)
        self.activate2 = nn.ELU()
        self.maxpool2 = nn.MaxPool2d((1,3), stride=(1,3))
        self.dropout2 = nn.Dropout(p=0.5)
        ...
        #Block3
        self.conv3 = Conv2dWithConstraint(50, 100, (1,5), max_norm=2, padding=0, bias=False)
        self.batchnorm3 = nn.BatchNorm2d(100)
        self.activate3 = nn.ELU()
        self.maxpool3 = nn.MaxPool2d((1,3), stride=(1,3))
        self.dropout3 = nn.Dropout(p=0.5)
        ...
        #Block4
        self.conv4 = Conv2dWithConstraint(100, 200, (1,5), max_norm=2, padding=0, bias=False)
        self.batchnorm4 = nn.BatchNorm2d(200)
        self.activate4 = nn.ELU()
        self.maxpool4 = nn.MaxPool2d((1,3), stride=(1,3))
        self.dropout4 = nn.Dropout(p=0.5)
        ...
        #Block5
        self.conv5 = Conv2dWithConstraint(200, 400, (1,5), max_norm=2, padding=0, bias=False)
        self.batchnorm5 = nn.BatchNorm2d(400)
        self.activate5 = nn.ELU()
        self.maxpool5 = nn.MaxPool2d((1,3), stride=(1,3))
    ...
    def output_feature_dim(self):
        data_tmp = torch.rand(1, 1, self.channels, self.points)
        EEGNetBackbone_tmp = DeepConveNet_6(self.channels, self.points, kernel_length=self.kernel_length, kernel_length2=self.kernel_length2, F1=self.F1, F2=self.F2, D=self.D, dropout_rate=0.5)
        EEGNetBackbone_tmp.eval()
        _feature_dim = EEGNetBackbone_tmp(data_tmp).view(-1,1).shape[0]
        return _feature_dim
...
class ShallowConveNet(nn.Module):
    ...
    def output_feature_dim(self):
        data_tmp = torch.rand(1, 1, self.channels, self.points)
        Backbone_tmp = ShallowConveNet(self.channels, self.points, kernel_length=self.kernel_length, kernel_length2=self.kernel_length2, F1=self.F1, F2=self.F2, D=self.D, dropout_rate=0.5)
        Backbone_tmp.eval()
        _feature_dim = Backbone_tmp(data_tmp).view(-1,1).shape[0]
        return _feature_dim
